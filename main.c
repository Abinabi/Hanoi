//  main.c
//  hanoi
//  Created by ABIN P on 24/05/18.
//  Copyright © 2018 ABIN P. All rights reserved.
...
void drawDisk(GLUquadricObj **quadric, const GLfloat outer, const GLfloat inner)      // disk design  position style
{
    glPushMatrix();
...
void drawPin(GLUquadricObj **quadric, const GLfloat radius, const GLfloat height)     //tower design position style
{
    glPushMatrix();
    glRotatef(-90.0, 1.0, 0.0, 0.0);
...
void setColor(const int color)  // set the colors for the each disks
{
    switch(color) {
        case 0:
            glColor3f(1.0, 0.0, 0.0); //for 1st disk
            break;
        case 1:
            glColor3f(0.0, 1.0, 0.0); //for 2nd disk
            break;
        case 2:
            glColor3f(1.0, 1.0, 0.0); //for 3rd disk
            break;
        case 3:
            glColor3f(0.0, 1.0, 1.0); // for 4th disk
            break;
        case 4:
            glColor3f(1.0, 0.0, 1.0); //for 5th disk
            break;
        case 5:
            glColor3f(0.0, 0.0, 0.0); //for rest \
            
            break;
    }
}

void Init(void) // used the light functions in the init method -specular,shininess,position
{
    ...
    glClearColor(1.0, 0.4, 0.2, 1.0);     // set screen clear color
...
void Key(unsigned char key, int x, int y)   // for set no of disk 1 to 9 and for controls used "s,x,-,+,f,r,q"
{
    switch(key)
    {
        case '1':
...
void SpecialKey(int key, int x, int y)     //use arrow keys to rotate the view
{
    switch(key)
    {
        case GLUT_KEY_UP:
...
void Display(void)
{
 ...
 //   drawBitmapString(-32.0, 8.0, -60.0, GLUT_BITMAP_9_BY_15, "Press 1 - 9 to select the number of disks");
    ...
    glColor3f(0.0, 0.0, 0.5);     //set the pins color
    
    drawAllPins(&quadric, config.pinradius, config.pinheight, config.gap);     // drawing the pins
